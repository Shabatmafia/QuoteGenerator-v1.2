...
def replace_cover_photo(prs: Presentation, photo_path) -> None:
    """Swap the large cover picture on slide 1 for a user-chosen image.

    The new image is placed into the existing picture frame, so position,
    size, and any overlapping text boxes are untouched — the layout stays
    exactly as designed. Photos close to the frame's shape look best.
    """
    if not photo_path:
        return
    slide = prs.slides[0]
    ...
    if not pictures:
        return
    # The cover image is the largest picture on the first slide
    target = max(pictures, key=lambda p: (p.width or 0) * (p.height or 0))
    _, rId = slide.part.get_or_add_image_part(str(photo_path))
    blip = target._element.blipFill.find(qn("a:blip"))
    if blip is not None:
